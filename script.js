  ...
  sources.forEach((source) => {
    for (let i = 0; i < quantity; i++) {
      ...
      if (!source.custom && source.printsUri) {
        ...
        button.classList.add("absolute", "b-2", "uppercase", "text-base", "btn", "backgroundbutton", "text-light");
...
const buildPdf = (
  ...
  cardPositions.forEach((position, i) => {
    ...
    doc.addImage(base64Images[i], "JPEG", x, y, cardWidth, cardHeight, undefined, 'FAST');
